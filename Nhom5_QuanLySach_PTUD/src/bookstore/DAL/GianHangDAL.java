...
//        System.out.println(GET_ALL);
        try {
            ...
            if (rs != null) {
                while (rs.next()) {
                    ...
                    item.setMaGianHang(rs.getString(1));
                    item.setTenGianHang(rs.getString(2));
                    item.setMoTa(rs.getString(3));
                    arr.add(item);
                }
            }
            closeConnection();
    ...
    public String getTenByID(String id) {
    	String s = null;
    	String sql = "select tenGianHang from tb_GianHang where tenGianHang = N'"+id+"'";
    	try {
			openConnection();
    ...
    public String getIdByName(String name) {
    	String s = null;
    	String sql = "select maGianHang from tb_GianHang where tenGianHang = ?";
    	try {
			openConnection();
			PreparedStatement ps = con.prepareStatement(sql);
			ps.setString(1, name);
